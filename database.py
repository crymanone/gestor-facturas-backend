# database.py - VERSIÓN FINAL CON CORRECCIÓN EN GET_INVOICE_DETAILS
import os
import psycopg2
...
def get_job_status(job_id, user_id):
    sql = "SELECT status, result_json, error_message FROM pdf_processing_queue WHERE id = %s AND user_id = %s;"
    conn = None
    try:
        conn = get_db_connection()
        cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        cur.execute(sql, (uuid.UUID(job_id), user_id))
        job = cur.fetchone()
        cur.close()
