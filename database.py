...
def get_job_status(job_id, user_id):
    ...
    try:
        ...
        # Buscar primero en la cola de PDFs
        sql_pdf = "SELECT status, result_json, error_message FROM pdf_processing_queue WHERE id = %s AND user_id = %s;"
        cur.execute(sql_pdf, (uuid.UUID(job_id), user_id))
        job = cur.fetchone()
        if job: return dict(job)
        
        # Si no se encuentra, buscar en la cola de imágenes
        sql_image = "SELECT status, result_json, error_message FROM image_processing_queue WHERE id = %s AND user_id = %s;"
...
def get_pending_job():
    # Intenta obtener un trabajo de PDF
    sql_pdf = "UPDATE pdf_processing_queue SET status = 'processing' WHERE id = (SELECT id FROM pdf_processing_queue WHERE status = 'pending' ORDER BY created_at LIMIT 1 FOR UPDATE SKIP LOCKED) RETURNING id, pdf_data AS data, user_id, 'pdf' as type;"
    conn = None
    try:
        conn = get_db_connection(); cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        cur.execute(sql_pdf); job = cur.fetchone()
        if job:
            conn.commit(); cur.close(); return dict(job)
        
        # Si no hay PDF, intenta obtener un trabajo de imagen
        sql_image = "UPDATE image_processing_queue SET status = 'processing' WHERE id = (SELECT id FROM image_processing_queue WHERE status = 'pending' ORDER BY created_at LIMIT 1 FOR UPDATE SKIP LOCKED) RETURNING id, image_data AS data, user_id, 'image' as type;"
        cur.execute(sql_image); job = cur.fetchone()
        conn.commit(); cur.close(); return dict(job) if job else None
    finally:
        if conn: conn.close()

def update_job_as_completed(job_id, result_json, job_type):
    table = "pdf_processing_queue" if job_type == 'pdf' else "image_processing_queue"
    data_column = "pdf_data" if job_type == 'pdf' else "image_data"
    sql = f"UPDATE {table} SET status = 'completed', result_json = %s, {data_column} = NULL WHERE id = %s;"
    conn = None
    try:
        conn = get_db_connection(); cur = conn.cursor()
        cur.execute(sql, (json.dumps(result_json), job_id)); conn.commit(); cur.close()
    finally:
        if conn: conn.close()
...
def update_job_as_failed(job_id, error_message, job_type):
    table = "pdf_processing_queue" if job_type == 'pdf' else "image_processing_queue"
    data_column = "pdf_data" if job_type == 'pdf' else "image_data"
    sql = f"UPDATE {table} SET status = 'failed', error_message = %s, {data_column} = NULL WHERE id = %s;"
    conn = None
    try:
        conn = get_db_connection(); cur = conn.cursor()
        cur.execute(sql, (error_message, job_id)); conn.commit(); cur.close()
    finally:
        if conn: conn.close()

# ... (resto de funciones como get_all_invoices, etc. sin cambios)
