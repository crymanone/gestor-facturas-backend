...
def init_db():
    ...
    try:
        ...
        conn.commit()
        cur.close()
        print("Base de datos y tablas (incluida la cola) listas.")
...
def create_pdf_job(pdf_data):
    job_id = str(uuid.uuid4())
    sql = "INSERT INTO pdf_processing_queue (id, status, pdf_data) VALUES (%s, 'pending', %s);"
    conn = None
    try:
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute(sql, (job_id, psycopg2.Binary(pdf_data)))
        conn.commit()
        cur.close()
        return job_id
    finally:
...
def get_pending_pdf_job():
    sql = "UPDATE pdf_processing_queue SET status = 'processing' WHERE id = (SELECT id FROM pdf_processing_queue WHERE status = 'pending' ORDER BY created_at LIMIT 1 FOR UPDATE SKIP LOCKED) RETURNING id, pdf_data;"
    conn = None
    try:
        conn = get_db_connection()
        cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
