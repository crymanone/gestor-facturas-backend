...
def process_queue():
    ...
    try:
        ...
        if len(content_parts) <= 1: raise ValueError("No se extrajo contenido suficiente.")
...
def handle_invoices():
    ...
    if request.method == 'POST':
        try:
            ...
            if not invoice_data or not invoice_data.get('emisor'): return jsonify({"ok": False, 'error': "Datos inválidos"}), 400
            if 'notas' not in invoice_data: invoice_data['notas'] = None
...
def handle_single_invoice(invoice_id):
    if request.method == 'GET':
        try:
            ...
            print(f"ERROR en endpoint /api/invoice/<id>: {e}"); return jsonify({"ok": False, "error": f"Error interno: {str(e)}"}), 500
...
def update_invoice_notes(invoice_id):
    try:
        ...
        return jsonify({"ok": True, "message": "Notas actualizadas"}) if success else jsonify({"ok": False, "error": "Factura no encontrada"}), 404
    except Exception as e: return jsonify({"ok": False, "error": f"Error interno: {str(e)}"}), 500
...
def ask_assistant():
    try:
        ...
        if not query_data or 'query' not in query_data: return jsonify({"ok": False, "error": "No se ha proporcionado ninguna pregunta."}), 400
        user_query = query_data['query']; all_invoices = db.get_all_invoices_with_details(g.user_id)
        if not all_invoices: return jsonify({"ok": True, "answer": "No tienes ninguna factura registrada todavía."})
        invoices_context = json.dumps(all_invoices, indent=2, ensure_ascii=False, default=str)
        prompt_contextual = f"""Actúa como un asistente experto en contabilidad.
        A continuación, te proporciono las facturas de un usuario en formato JSON.
        Tu tarea es responder a la pregunta del usuario basándote únicamente en estos datos.
        DATOS DE LAS FACTURAS: ```json\n{invoices_context}\n```
        PREGUNTA DEL USUARIO: "{user_query}"
        Proporciona una respuesta clara y directa. No inventes datos.
        """
